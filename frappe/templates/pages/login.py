...
WEIXIN_CORPID = social.facebook_client_id or ""
WEIXIN_CORPSECRET = social.facebook_client_secret or ""
...
WEIXIN_ACCESSTOKEN_ADDR = "https://qyapi.weixin.qq.com/cgi-bin/gettoken?corpid=" + \
    WEIXIN_CORPID + "&corpsecret=" + WEIXIN_CORPSECRET
...
def get_oauth_keys(provider):
    ...
    if not keys:
        ...
        for fieldname in ("client_id", "client_secret"):
            value = social.get(
                "{provider}_{fieldname}".format(provider=provider, fieldname=fieldname))
            if not value:
                keys = {}
                break
...
def login_oauth_user(data, provider=None):
    ...
    frappe.local.response[
        "location"] = "/desk#desktop" if frappe.local.response.get('message') == 'Logged In' else "/"
